...
public class FenseHP_2 : MonoBehaviour {
    ...
    void Start()
    {
        if (DataController.instance.gameData.fense_HP_2 <= 0)
        {
            DataController.instance.gameData.fense_HP_2 = fenseHP_Max;
        }
        if (DataController.instance.gameData.fense_HP_2 > 0)
        {
    ...
    { // 펜스의 체력이 0이하로 떨어지면 오브젝트의 모습이 보이지 않도록 렌더링 비활성화
        ...
        fenseHP = DataController.instance.gameData.fense_HP_2;
        if (DataController.instance.gameData.fense_HP_2 >= fenseHP_Max)
        {
            DataController.instance.gameData.fense_HP_2 = fenseHP_Max;
        }
    }
    ...
    {// 디펜스 몬스터에게 닿을경우 펜스의 체력감소
        if (coll.transform.tag == "defense_enemy")
        {
            ...
            {
                DataController.instance.gameData.fense_HP_2 -= 1f;
                if (DataController.instance.gameData.fense_HP_2 <= 0)
                {
